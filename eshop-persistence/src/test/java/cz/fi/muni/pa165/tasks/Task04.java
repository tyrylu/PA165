...
public class Task04 extends AbstractTestNGSpringContextTests {
	...
	public void groupByAndOrderBySelectNew() {
		List<ColorCount> colorCounts = em.createQuery("select new cz.muni.fi.pa165.ColorCount(p.color, count(p)) from Product p group by p.color order by p.color",ColorCount.class).getResultList();
	...
	public void leftJoinCagesWithPets() {
		...
		List<ProductAndCategory> productAndCategory = em.createQuery("select new cz.muni.fi.pa165.ProductAndCategory(p, c) from Product p left join p.categories c on c.name = Kitchen",ProductAndCategory.class).getResultList();
